...
class VCOCOeval(object):
    ...
    def __init__(self, vcocodb,vsrl_annot_file, coco_annot_file,
                 split_file):
        """Input:
    ...
    def _get_vcocodb(self):
        vcocodb = json.load(open(self.vcocodb, 'rb'))
        return vcocodb
    ...
    def _do_role_eval(self, vcocodb, detections_file, ovr_thresh=0.5, eval_type='scenario_1'):
        ...
        #     dets = pickle.load(f)
        with open(detections_file, 'rb') as f:
            dets = json.load(f, encoding='utf-8')
        ...
        for i in range(len(vcocodb)):
            ...
            gt_inds = np.where(np.array(vcocodb[i]['gt_classes']) == 1)[0]
            # person boxes
            gt_boxes = np.array(vcocodb[i]['boxes'])[gt_inds]
            gt_actions = np.array(vcocodb[i]['gt_actions'])[gt_inds]
            ...
            for aid in range(self.num_actions):
                ...
                for rid in range(len(self.roles[aid]) - 1):
                    ...
                    gt_role_inds = np.array(vcocodb[i]['gt_role_id'])[gt_inds, aid, rid]
                    ...
                    for j in idx:
                        ...
                        if is_true_action and (ovmax >= ovr_thresh) and (ov_role >= ovr_thresh):
                            if covered[jmax]:
                                ...
                                fp[aid][rid].append(0)
                                tp[aid][rid].append(1)
                                # print('True positive', aid, rid, image_id)
                                # print('human box', pred_box)
                                # print('object box', role_boxes[j, :], '\n')
                                covered[jmax] = True
                        else:
                            # if not is_true_action and (ovmax>=ovr_thresh) and (ov_role >= ovr_thresh):
                            # print('false positive (false action)', aid, rid, image_id)
                            # print('human box', pred_box)
                            # print('object box', role_boxes[j, :], '\n')
                            fp[aid][rid].append(1)
                            tp[aid][rid].append(0)
        ...
        print('---------------------------------------------')
        print('Average Role [%s] AP = %.2f, omitting the action "point"' % (
